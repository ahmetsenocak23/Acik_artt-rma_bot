...
class DatabaseManager:
    ...
    def get_prize_img(self, prize_id):
        ...
        with conn:
            ...
            cur.execute("""SELECT image 
                            FROM prizes 
                            WHERE prize_id = ? """,(prize_id,)) # Corrected line
            
            result = cur.fetchone()  # Fetch one result or None
            if result:
                return result[0]  # Return the image path if found
            else:
    ...
    def get_random_prize(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("""SELECT * FROM prizes 
                                 WHERE used = 0
                                 ORDER BY RANDOM()
                                 LIMIT 1 """
                                 )
...
def hide_img(img_name):
    ...
    cv2.imwrite(f'nexto/TUR-PythonLVL3-M4L1/hidden_img/{img_name}', pixelated_image)

if __name__ == '__main__':
    manager = DatabaseManager(DATABASE)
    manager.create_tables()
    prizes_img = os.listdir('nexto/TUR-PythonLVL3-M4L1/img')
